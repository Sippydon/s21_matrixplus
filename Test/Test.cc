...
TEST(Constructor_param, test_6) { ASSERT_ANY_THROW(S21Matrix M(-2, 5)); }

TEST(Constructor_param, test_7) { ASSERT_ANY_THROW(S21Matrix M(-1, 0)); }
...
TEST(Method_Determinant, test_6) {
  S21Matrix M;
  M.SetCols(1);
  ASSERT_ANY_THROW(M.Determinant());
}
...
TEST(Method_SetRows, test_4) {
  S21Matrix M;
  ASSERT_ANY_THROW(M.SetRows(-2));
}

TEST(Method_SetCols, test_3) {
  S21Matrix M;
  M.SetCols(2);
  ASSERT_EQ(M.GetRows(), 0);
...
TEST(Method_SetCols, test_4) {
  S21Matrix M;
  ASSERT_ANY_THROW(M.SetCols(-2));
}
